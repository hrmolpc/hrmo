<div>
    @section('title', 'Payslip Requests')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('Payslip Requests') }}</li>
        </ol>
    </nav>
    ...
    <div class="row">
        <div class="col-12 mb-4">
            <div class="row mt-3">
                <div class="col">
                    <div class="card">
                        <h5 class="card-header">{{ __('Payslip Requests')}}</h5>
                        <div class="table-responsive text-nowrap">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody class="table-border-bottom-0">
                                    @forelse($payslipRecords as $payslip)
                                        <tr>
                                            <td><strong>{{ $payslip->id }}</strong></td>
                                            <td class="td">{{ $this->getEmployeeName($payslip->employee_id) }}</td>
                                            <td style="text-align: center">{{ $payslip->request_date }}</td>
                                            <td style="text-align: center">{{ $payslip->status }}</td>
                                            <td style="text-align: center">
                                                ...
                                                <button type="button" class="btn btn-sm btn-success rounded-pill btn-icon waves-effect">
                                                    <span wire:click.prevent="showEditPayslipModal({{ $payslip->id }})" data-bs-toggle="modal" data-bs-target="#payslipModal" class="ti ti-check"></span>
                                                </button>
                                                <button type="button" class="btn btn-sm btn-tr rounded-pill btn-icon btn-outline-danger waves-effect">
                                                    <span wire:click.prevent="confirmDestroyPayslip({{ $payslip->id }})" class="ti ti-trash"></span>
                                                </button>
                                                @if ($confirmedId === $payslip->id)
                                                    <button wire:click.prevent="destroyPayslip" type="button" class="btn btn-xs btn-danger waves-effect waves-light">
                                                        {{ __('Sure?') }}
                                                    </button>
                                                @endif
                                            </td>
